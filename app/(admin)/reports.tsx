...
function DatePickerModal({
  ...
  const isValid = /^\d{4}-\d{2}-\d{2}$/.test(text) && !isNaN(Date.parse(text + 'T00:00:00'));
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            ...
            <TextInput
              ...
              style={{
                ...
                borderRadius: 8,
                borderWidth: 1,
                borderColor: isValid ? COLORS.borderColor : COLORS.error,
...
export default function ReportsScreen() {
  ...
  const salaryRows = workers.filter((w) => w.active).map(makeRow);
  const removedRows = workers.filter((w) => !w.active).map(makeRow);
